...
The following plots represent the Gender and Vehicle Age bar percentages.
...
p20 <- ggplot(train_reduced, aes(x = Vehicle_Age, fill = Response)) + 
  geom_bar(position = "fill", color = "black") +
  labs(y = "Percentage", fill = "Response") +
  scale_y_continuous(labels = scales::percent_format()) +
  theme(
...
```
We can also see that people that have kept the car for a longer interval of time are more likely to get an insurance.


For the other variables we have
```{r}
p21 <- ggplot(train_reduced, aes(x = Driving_License, fill = Response)) + 
  geom_bar(position = "fill", color = "black") +
  labs(y = "Percentage", fill = "Response") +
  scale_y_continuous(labels = scales::percent_format()) +
  theme(
...
p22 <- ggplot(train_reduced, aes(x = Vehicle_Damage, fill = Response)) + 
  geom_bar(position = "fill", color = "black") +
  labs(y = "Percentage", fill = "Response") +
  scale_y_continuous(labels = scales::percent_format()) +
  theme(
...
p23 <- ggplot(train_reduced, aes(x = Previously_Insured, fill = Response)) + 
  geom_bar(position = "fill", color = "black") +
  labs(y = "Percentage", fill = "Response") +
  scale_y_continuous(labels = scales::percent_format()) +
  theme(
...
difference between TPR and FPR is around 0.03; this small value could be
...
It's also worth to mention that even if in the other models the variable Vehicle_Damage had a high effect on the response variable, in random forest it is not very important for the performances of the model.
